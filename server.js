...
app.get('/delete', (req, res) => {
    ...
    dbConnect.query('delete from comment where post_id=?', [postId]);
...
app.get('/cmt_delete', (req, res) => {
    let postId = req.query.id;
    let cmtId = req.query.cmtid;
    console.log('cmt id: '+cmtId+ ', postid: '+postId);

    dbConnect.query('delete from comment where post_id=? and cmt_id=?', [postId, cmtId], (error) => {
        if (error) throw error;
    });
...
app.get('/like', (req, res) => {
    ...
    if (req.session.user) {
        ...
        dbConnect.query('select * from like_table where user_nickname=? and post_id=?', [nickname, postId], (error, rows) => {
            if (error) throw error;
            if (rows.length == 0) { //현재 유저가 글에 좋아요를 안눌렀으면
                dbConnect.query('insert into like_table value (?,?)', [nickname, postId], (error) => {
                    if (error) throw error;
                });
                dbConnect.query('update post set like_cnt=like_cnt+1 where post_id=?', [postId]);
            }
            else { //좋아요가 이미 눌러져있다면
                dbConnect.query('update post set like_cnt=like_cnt-1 where post_id=?', [postId]);
                dbConnect.query('delete from like_table where user_nickname=? and post_id=?', [nickname, postId], (error) => {
                    if (error) throw error;
                })
            }
        });
